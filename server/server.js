...
const verifyJWT = (req, res, next) => {
  ...
  jwt.verify(token, process.env.DB_LOCATION, (err, user) => {
    if (err) {
...
server.post("/latest-blogs", (req, res) => {
  ...
  console.log("latest-blog is hit")
  let maxLimit = 4;
  let { page } = req.body;
...
server.post("/create-blog", verifyJWT, (req, res) => {
  ...
  let { title, banner, content, des, draft, tags, id } = req.body;
  ...
  if (!draft) {
    ...
    if (!content.blocks.length) {
      return res.status(403).json({
        error: "You must be some blog content to publish it",
      });
    }
...
server.post("/get-blog", (req, res) => {
  ...
  Blog.findOneAndUpdate(
    ...
    .select("title des banner content tags publishedAt activity blog_id")
    .then((blog) => {
      User.findOneAndUpdate(
